...
namespace SYSFindLicEaseBoardComplainantSubject
{
    ...
    public class HRInspectionLookup : IWorkflowScript
    {
        ...
        private const string ScriptName = "416 - ABT - Inspection Lookup to VR";
        ...
        private const string gSaveToInspTypeDesc = "Inspection Type (ABT-FL)";
        ...
        public void OnWorkflowScriptExecute(Hyland.Unity.Application app, Hyland.Unity.WorkflowEventArgs args)
        {
            try
            {
                ...
                //get and clean Inspection Visit ID keyword for passing to LicEase database
                KeywordType kwtInsptNum = _currentDocument.DocumentType.KeywordRecordTypes.FindKeywordType(gSaveToInspectionID);
                string strInsptNum = "";
                if (kwtInsptNum != null)
                {
                    KeywordRecord keyInsptNum = _currentDocument.KeywordRecords.Find(kwtInsptNum);
                    if (keyInsptNum != null)
                    {
                        Keyword kwdInspectionDate = keyInsptNum.Keywords.Find(kwtInsptNum);
                        if (kwdInspectionDate != null)
                            strInsptNum = CleanSeedKW(kwdInspectionDate.ToString());
                    }
                }
				...
				*/
                ...
                strSql.Append(@" FROM (SELECT DISTINCT TO_CHAR (insp_hist.insp_nbr) AS insp_nbr, TO_CHAR (insp_vst.insp_vst_id) AS insp_vst_id, lic.clnt_cde AS lic_type, ");
                strSql.Append(@" lic.xent_id AS indorg_num, lic.lic_nbr AS lic_num, lic.file_nbr as file_num, TO_CHAR (insp_vst.insp_vst_nbr) AS visit_num, ");
                ...
                strSql.Append(@" stff.frst_nme || '.' || stff.surnme AS inspector_name ,insp_hist.lic_id as insp_lic_id, ");
                strSql.Append(@" (select max(key_nme) from name n, link k where k.link_id = insp_hist.link_id and k.nme_id = n.nme_id) as key_name, ");
                strSql.Append(@" (SELECT MAX (key_nme) FROM NAME n, LINK k WHERE k.prnt_id = insp_hist.lic_id and k.link_prnt_cde = 'L' and k.curr_ind = 'Y' ");
                ...
                strSql.Append(@" (select  insp_regn_cde from insp_regn r, link k where k.link_id = insp_hist.link_id and k.insp_regn_id = r.insp_regn_id) as region_kw");
                strSql.Append(@" FROM insp_vst, insp_hist, insp_typ_defn,  insp_disp_typ, inspr, stff, lic WHERE insp_hist.insp_hist_id = insp_vst.insp_hist_id ");
                ...
                strSql.Append(strInsptNum);
                strSql.Append(@"') AND insp_typ_defn.insp_typ_defn_id = insp_hist.insp_typ_defn_id AND insp_vst.inspr_id = inspr.inspr_id AND ");
                strSql.Append(@" stff.stff_oper_id = inspr.stff_oper_id AND lic.lic_id = insp_hist.lic_id AND insp_disp_typ.insp_disp_typ_id = insp_hist.insp_disp_typ_id) a ");
                    ...
                    try
                    {
                        ...
                        using (OdbcDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                Keyword kwdVisitDate = null;
                                if (!String.IsNullOrEmpty(strInspDate))
                                {
                                    KeywordType kwtVisitDate = app.Core.KeywordTypes.Find(gSaveToInspectVisitDate);
                                    if (kwtVisitDate != null)
                                        kwdVisitDate = CreateKeywordHelper(kwtVisitDate, strInspDate);
                                }
                                ...
                                using (DocumentLock documentLock = _currentDocument.LockDocument())
                                {
                                    ...
                                    if (kwdVisitDate != null) keyModifier.AddKeyword(kwdVisitDate);
                                    ...
                                    string output = String.Format("Keyword: '{0}' Value: '{1}', {33}Keyword: '{2}' Value: '{3}', {33}Keyword: '{4}' Value: '{5}', {33}Keyword: '{6}' Value: '{7}'," +
                                        ...
                                        gSaveToLicNum, strLicenseNum, gSaveToLicType, strLicType, gSaveToInspectionID, strInspID, gSaveToFileNum, strFileNum, gSaveToKeyName, strKeyName, gSaveToSubject, strSubject,
                                        ...
                                        gSaveToRegion, strRegion, gSaveToInspectorName, strInspector, gSaveToInspTypeDesc, strInspType, gSaveToInspectVisitDate, strInspDate, _currentDocument.ID, Environment.NewLine);
