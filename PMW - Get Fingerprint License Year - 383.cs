...
namespace SYSFindLicEaseBoardComplainantSubject
{
    ...
    public class HRInspectionLookup : IWorkflowScript
    {
        ...
        private const string ScriptName = "383 - PMW - Get Fingerprint License Year";
        ...
        private const string gSaveToLicYear = "License Year";
        ...
        public void OnWorkflowScriptExecute(Hyland.Unity.Application app, Hyland.Unity.WorkflowEventArgs args)
        {
            try
            {
                ...
                strSql.Append(@"select distinct to_char(sysdate,'YY') || '/' || (to_char(sysdate,'YY') + 3) as YEAR3, to_char(sysdate,'YY') || '/' || (to_char(sysdate,'YY') + 5) as YEAR5, ");
                strSql.Append(@" vw_appl.clnt_cde from vw_appl as LICTYPE, vw_lic where vw_appl.clnt_cde = '");
                strSql.Append(strLicenseType);
                strSql.Append(@"' and vw_appl.applc_nbr = '");
                strSql.Append(strAppNum);
                strSql.Append(@"' and vw_lic.lic_id = vw_appl.lic_id and vw_appl.xact_cde = 2020");
                strSql.Append(@"')");
                    ...
                    try
                    {
                        ...
                        using (OdbcDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                strLicType = reader["LICTYPE"].ToString();
                                strYear3 = reader["YEAR3"].ToString();
                                strYear5 = reader["YEAR5"].ToString();

                                if (strLicType == "1051" || strLicType == "1052" || strLicType == "1053" || strLicType == "1054" || strLicType == "1055" || strLicType == "1056")
                                {
                                    strYear = strYear3;
                                }
                                else if (strLicType == "1012" || strLicType == "1013" || strLicType == "1021" || strLicType == "1022" || strLicType == "1031" || strLicType == "1032")
                                {
                                    strYear = strYear5;
                                }
                                else
                                {
                                    throw new Exception(string.Format("License Type does not match required value for lookup"));
                                }
                                
                                Keyword kwdLicenseYear = null;
                                if (!String.IsNullOrEmpty(strYear))
                                {
                                    KeywordType kwtLicenseYear = app.Core.KeywordTypes.Find(gSaveToLicYear);
                                    if (kwtLicenseYear != null)
                                        kwdLicenseYear = CreateKeywordHelper(kwtLicenseYear, strYear);
                                }
                                ...
                                using (DocumentLock documentLock = _currentDocument.LockDocument())
                                {
                                    ...
                                    if (kwdLicenseYear != null) keyModifier.AddKeyword(kwdLicenseYear);


                                    // Apply keyword change to the document
                                    keyModifier.ApplyChanges();

                                    string output = String.Format("Keyword: '{0}' Value: '{1}', added to Document {2}.", gSaveToLicYear, strYear, _currentDocument.ID);
