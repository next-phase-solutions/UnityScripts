...
namespace CRDEMailAddressLookUp
{
    ...
    public class CRDEMailAddressLookUp : IWorkflowScript
    {
        ...
        private const string ScriptName = "446 - ABT - Quota County Lookup;
        ...
        private const string gSaveToLocMDF = "Email Address";
        ...
        public void OnWorkflowScriptExecute(Hyland.Unity.Application app, Hyland.Unity.WorkflowEventArgs args)
        {
            try
            {
				...
				*/
                ...
                strSql.Append(@"select m.mod_desc as Location_Modifier_App ");
                strSql.Append(@"  from appl a, appl_mdf am, mdf m ");
                strSql.Append(@"  where a.applc_id = am.applc_id");
                strSql.Append(@"  and am.mod_id = m.mod_id and m.mod_typ = 'L' ");
                    ...
                    try
                    {
                        ...
                        using (OdbcDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                strLocMOD = reader["Location_Modifier_App"].ToString();

                                Keyword kwdLocMOD = null;
                                if (!String.IsNullOrEmpty(strLocMOD))
                                {
                                    KeywordType kwtLocMOD = app.Core.KeywordTypes.Find(gSaveToLocMDF);
                                    if (kwtLocMOD != null)
                                        kwdLocMOD = CreateKeywordHelper(kwtLocMOD, strLocMOD);
                                }
                                ...
                                using (DocumentLock documentLock = _currentDocument.LockDocument())
                                {
                                    ...
                                    if (kwdLocMOD != null) keyModifier.AddKeyword(kwdLocMOD);
                                    ...
                                    string output = String.Format("Keyword: '{0}' Value: '{1}', {3}added to Document {2}.",
                                        gSaveToLocMDF, strLocMOD, _currentDocument.ID, Environment.NewLine);
